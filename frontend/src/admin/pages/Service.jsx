  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-extrabold text-[#0b2545]">Services</h1>
          <div className="w-12 h-1 bg-[#D7B25B] mt-2 rounded-full" />
        </div>
        ...
        <table className="w-full">
          <thead className="bg-gray-50 border-b border-gray-100">
            <tr>
              ...
              <th className="p-4 text-left text-xs font-bold text-gray-500 uppercase">Service Title</th>
          ...
          <tbody className="divide-y divide-gray-50">
            {services.map((item) => (
              <tr key={item.id} className="hover:bg-gray-50/50">
                ...
                <td className="p-4 font-semibold text-[#0b2545]">{item.title}</td>
                <td className="p-4">
        ...
        <div className="fixed inset-0 bg-[#0b2545]/20 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl p-8 animate-in fade-in zoom-in duration-200">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-extrabold text-[#0b2545]">{editingId ? "Edit Service" : "Add New Service"}</h2>
              <button onClick={() => setOpenModal(false)} className="text-gray-400 hover:text-red-500"><FiX size={24} /></button>
            </div>
            <form onSubmit={handleSubmit} className="space-y-4">
              <input type="text" placeholder="Title" value={formData.title} onChange={(e) => setFormData({...formData, title: e.target.value})} className="w-full border border-gray-200 rounded-xl p-3 focus:ring-2 focus:ring-[#0b2545] outline-none" required />
              <textarea placeholder="Short Description" value={formData.shortDescription} onChange={(e) => setFormData({...formData, shortDescription: e.target.value})} className="w-full border border-gray-200 rounded-xl p-3" rows="3" />
              <input type="file" onChange={(e) => setFormData({...formData, image: e.target.files[0]})} className="w-full p-2 border-2 border-dashed border-gray-200 rounded-xl" />
              <button type="submit" className="w-full bg-[#0b2545] text-white py-3 rounded-xl font-bold hover:bg-[#153a66] mt-4">Save Changes</button>
            </form>
          </div>
        </div>
      )}
